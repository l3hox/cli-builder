...
echo "    openai-cli open-ai-model get-model --model gpt-4o --json --api-key YOUR_KEY"
echo ""
...
if [ -n "$API_KEY" ]; then
    ...
    echo "$ openai-cli open-ai-model get-model --model gpt-4o --json --api-key sk-..."
    dotnet run --project "$CLI" --no-build -- open-ai-model get-model --model gpt-4o --json --api-key "$API_KEY"
    echo ""
else
    echo "$ openai-cli --help"
    dotnet run --project "$CLI" --no-build -- --help
