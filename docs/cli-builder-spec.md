...
| [023](docs/ADR.md#adr-023-single-client-sdk-shape-discovery-via-verb-noun-method-grouping) | Single-client SDK shape discovery via verb-noun method grouping |
...
**v0.2.2 is current.** See [docs/FUTURE.md](FUTURE.md) for the full prioritized roadmap going forward. The spec does not mirror it — that would just drift.
...
- **v0.2.2:** Single-client SDK shape discovery in the Python adapter ([ADR-023](ADR.md#adr-023-single-client-sdk-shape-discovery-via-verb-noun-method-grouping)). Verb-noun method grouping with whitelist-based filtering; new `_naming.py` module; `SdkMetadata.discovery_mode` provenance + `pypi_name` distribution-vs-import-name field; new `--entry-class` CLI flag. PyGithub now extracts 33 resources, end-to-end validated against `api.github.com`. 707 total tests.
